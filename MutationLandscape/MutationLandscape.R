...
F1<-read.table("F1_MutGene_Matrix.txt", sep="\t", quote="", header=TRUE, row.names=1)
F2<-read.table("F2_MutGene_Stats.txt", sep="\t", quote="", header=TRUE, row.names=1)
F3<-read.table("F3_NumMut_Sorted.txt", sep="\t", quote="", header=TRUE, row.names=1)
...
axis(2,at=0.5:(nrow(F1)-0.5), labels = rev(row.names(F2)),las=1, cex.axis=1.4, font=1, line=FALSE,tick=FALSE)
